...
#modify
modify1=('修改' '退出' '请输入学号' '学号为4位' '该学生' '不存在' '修改学生' '记录' '格式' '数据库' 'C语言' '数据结构' '百分制' '举例' '输入应有3个字段' '成绩为百分制' '成绩为非负数' '是否确定' '修改成功' '无效选择')
modify2=('Modify' 'Quit' 'Please type number> ' 'The length of number should be 4' 'The student' 'is not existed' 'Modify student' 'record' 'Format' 'Database' 'C-language' 'DataStructure' 'Hundred-mark System' 'e.g.' 'Request three fields' 'Hundred-mark System' 'Non-negative' 'Are you sure' 'Modify successfully!' 'Invalid choice')

#delete
delete1=('删除' '退出' '请输入学号' '学号为4位' '该学生' '不存在' '确定删除学生' '删除成功' '无效选择')
delete2=('Delete' 'Quit' 'Please type number' 'The length of number should be 4' 'The student' 'is not existed' 'Are you sure to remove student' 'Remove successfully!' 'Invalid choice')

#query
query1=('查询' '退出' '请输入学号' '学号为4位' '该学生' '不存在' '格式' '学号' '姓名' '数据库' 'C语言' '数据结构' '平均成绩' '修改时间' '是否查询学生信息' '格式' '学号' '姓名' '性别' '班级' '修改时间' '无效选择') 
query2=('Query' 'Quit' 'Please type number' 'The length of number should be 4' 'The student' 'is not existed' 'Format' 'SNo' 'Name' 'Database' 'C-language' 'DataStructure' 'Average' 'Modified-date' 'Whether to query student Information' 'Format' 'SNo' 'Name' 'Gender' 'Class' 'Modified-date' 'Invalid choice')
